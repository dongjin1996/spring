...
class ItemRepositoryTest {
	...
	@PersistenceContext  //영속성(지속성) 컨텍스트
	...
	@Disabled
	@Test
	@DisplayName("@Query 퀴즈2")
	public void findByitemNmByitemSellStatus() {
		this.createItemList();
		List<Item> itemList = itemRepository.findByItemNmAndItemSellStatus("테스트 상품1", ItemSellStatus.SELL);
	...
	public void queryDslTest() {
		...
		List<Item> itemList = query.fetch(); //fetch = 쿼리문 실행
	...
	public void queryDslTest2() {
		...
		//0부터 페이지 번호가 시작된다.
		Pageable page = PageRequest.of(1, 3);  //of (조회할 페이지의 번호, 페이지당 조회할 데이터의 갯수)
	...
	public void queryDslQuiz3() {
		...
		JPAQuery<Item> query = qf.selectFrom(qItem)
								.where(qItem.regTime.gt(LocalDateTime.of(2023, 1, 1, 12, 12, 44)));
