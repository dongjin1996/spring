...
public class BoardController {
	...
	//get방식으로 Request가 들어올때 
	//게시글 등록
	//게시글 작성하고 등록하면 POST 방식으로 
	@RequestMapping(value = "/created", method = RequestMethod.POST) //localhost 로 접속
	public String createdOK(Board board, HttpServletRequest request, Model model) {
		
		try {
			int maxNum = boardService.maxNum();
			
			board.setNum(maxNum + 1); //num컬럼에 들어갈 값을 1증가 시켜준다.
			...
			e.printStackTrace();
			model.addAttribute("errorMessage", "게시글 작성중에 오류가 발생했습니다." );
			return "bbs/created";
		}
	...
	@RequestMapping(value = "/list", method = {RequestMethod.GET, RequestMethod.POST}) //localhost 로 접속
	public String list(Board board, HttpServletRequest request, Model model) {
		...
		try {
			...
			if(searchValue == null) {
				searchKey = "subject"; //검색키워드를 디폴트 subject 
			...
			if (currentPage > totalPage) currentPage = totalPage; //currentPage는 totalPage보다 크면 안된다
			...
			e.printStackTrace();
			model.addAttribute("errorMessage", "리스트를 불러오는중 에러가 발생 했습니다." );
		}
	...
	public String article(HttpServletRequest request, Model model) {
		try {
			...
			e.printStackTrace();
			model.addAttribute("errorMessage", "게시글을 불러오는중 에러가 발생 했습니다." );
		}
		return "bbs/article";
	}
